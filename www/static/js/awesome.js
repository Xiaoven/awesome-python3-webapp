        ...
        showFormLoading: function (isLoading) {
            return this.each(function () {
                ...
                $span = $submit && $submit.find('span'),
                    iconClass = $span && $span.attr('uk-icon');
                if (!$form.is('form')) {
                    console.error('Cannot call showFormLoading() on non-form object.');
                    return;
                }
                if (!iconClass) {
                    console.warn('Icon <span uk-icon="*" not found.');
                    return;
                }
                if (isLoading) {
                    $buttons.attr('disabled', 'disabled');
                    $span && $span.addClass('uk-icon-spinner').addClass('uk-icon-spin');
                } else {
                    $buttons.removeAttr('disabled');
                    $span && $span.removeClass('uk-icon-spinner').removeClass('uk-icon-spin');
                }
            });
        },
...
if (typeof (Vue) !== 'undefined') {
    ...
    Vue.component('pagination', {
        template: '<ul class="uk-pagination">' +
            '<li v-if="! has_previous" class="uk-disabled"><span><span uk-icon="chevron-double-left"></span></span></li>' +
            '<li v-if="has_previous"><a v-attr="onclick:\'gotoPage(\' + (page_index-1) + \')\'" href="#0"><span uk-icon="chevron-double-left"></span></a></li>' +
            '<li class="uk-active"><span v-text="page_index"></span></li>' +
            '<li v-if="! has_next" class="uk-disabled"><span><span uk-icon="chevron-double-right"></span></span></li>' +
            '<li v-if="has_next"><a v-attr="onclick:\'gotoPage(\' + (page_index+1) + \')\'" href="#0"><span uk-icon="chevron-double-right"></span></a></li>' +
            '</ul>'
    });
}
